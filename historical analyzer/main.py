...
def create_price_chart(data, ticker):
    """
    Create line chart showing closing prices with moving averages
    ...
    ax.plot(data.index, data['SMA_7'], label='7-Day SMA', color='#ff7f0e', linewidth=1.5, linestyle='--', alpha=0.7)
    ax.plot(data.index, data['SMA_14'], label='14-Day SMA', color='#2ca02c', linewidth=1.5, linestyle='--', alpha=0.7)
    
    # Formatting
    ax.set_title(f'{ticker} - 30 Day Price Analysis', fontsize=16, fontweight='bold', pad=20)
...
def create_candlestick_chart(data, ticker):
    ...
    mpf.plot(data, type='candle', style=s, volume=True, 
             title=f'{ticker} - Candlestick Chart (30 Days)',
             ylabel='Price (₹)', ylabel_lower='Volume',
             figsize=(12, 8), savefig=filename)
...
def create_volume_chart(data, ticker):
    ...
    # Formatting
    ax.set_title(f'{ticker} - Trading Volume (30 Days)', fontsize=16, fontweight='bold', pad=20)
    ax.set_xlabel('Date', fontsize=12, fontweight='bold')
...
def export_to_csv(data, ticker):
    ...
    try:
        timestamp = datetime.now().strftime('%Y_%m_%d')
        ticker_clean = ticker.replace('.', '_')
        filename = f"{OUTPUT_FOLDER}/{ticker_clean}_data_{timestamp}.csv"
...
def analyze_stock(ticker):
    ...
    try:
        ...
        # Export to CSV
        csv_file = export_to_csv(data, ticker)
